...
#define kCircleMarginSmall  32.0              // 圆点离屏幕左边距（4 5）
#define kCircleMarginCenter 43.0              // 圆点离屏幕左边距（6）
#define kCircleMarginbig    70.0              // 圆点离屏幕左边距（6+）
...
- (void)initCircles
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (kSCREEN_WIDTH==320) {
            x = kCircleMarginSmall + (i%3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginSmall*2- kCircleDiameter *3)/2);
            y = kCircleMarginSmall + (i/3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginSmall*2- kCircleDiameter *3)/2);
        }else if (kSCREEN_WIDTH==375)
        {
            x = kCircleMarginCenter + (i%3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginCenter*2- kCircleDiameter *3)/2);
            y = kCircleMarginCenter + (i/3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginCenter*2- kCircleDiameter *3)/2);
        }else if (kSCREEN_WIDTH==414)
        {
            x = kCircleMarginbig + (i%3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginbig*2- kCircleDiameter *3)/2);
            y = kCircleMarginbig + (i/3) * (kCircleDiameter+(kSCREEN_WIDTH-kCircleMarginbig*2- kCircleDiameter *3)/2);
        }
...
//    if (selectedButtonArray.count<5) {
//        UIViewController *controller=[[UIViewController alloc] init];
//        [controller showWarmingWithMessage:@"密码太短，需要六位"];
//        return;
//         [self clearColorAndSelectedButton];
//    }
//    else{
...
- (void)showErrorCircles:(NSString*)string
{
    ...
    for (int i = 0; i < string.length; i++) {
        ...
        [_selectedButtonArray addObject:buttonArray[number.intValue]];
    }
    
    for (UIButton* button in buttonArray) {
        if (button.selected) {
            [button setBackgroundImage:[UIImage imageNamed:@"circle_wron"] forState:UIControlStateSelected];
        }
